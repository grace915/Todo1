...
public class MyRemoteViewsFactory implements RemoteViewsService.RemoteViewsFactory {
    ...
    public List<TodoItem> arrayList;
    ...
    public void setData() {
        ...
        arrayList = myDatabase.todoDao().getAllTodo();
    ...
    public RemoteViews getViewAt(int position) {
        RemoteViews listviewWidget = new RemoteViews(context.getPackageName(), R.layout.item_todo);
        listviewWidget.setTextViewText(R.id.widget_tv_title, arrayList.get(position).getTitle());
        listviewWidget.setTextViewText(R.id.widget_tv_due, arrayList.get(position).getDue_date());

        // 항목 선택 이벤트 발생 시 인텐트에 담겨야 할 항목 데이터를 추가해주는 코드
        Intent dataIntent = new Intent();
        dataIntent.putExtra("item_id", arrayList.get(position).getId());
        dataIntent.putExtra("item_data", arrayList.get(position).getTitle());
        listviewWidget.setOnClickFillInIntent(R.id.widget_listview, dataIntent);
